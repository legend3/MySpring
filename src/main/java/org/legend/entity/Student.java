...
public class Student {
    ...
    public void learnJava(){
        ICourse course = new JavaCourse();//通过new创建
        course.learn();
    }
    public void learnHtml(){
        ICourse course = new HtmlCourse();//通过new创建
        course.learn();
    }

    public void learnCourse(String name) {
//        ICourse course = CourseFactory.getCourse(name);//通过工厂获取(learnCourseWithFactory)
        ApplicationContext context = new ClassPathXmlApplicationContext("applicationContext.xml");//通过spring容器创建获取
